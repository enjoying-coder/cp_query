...
constants AS (
    SELECT
        TIMESTAMP '2025-09-25' AS startDate,
        40 AS min_buy_mc,
        10000 AS max_buy_mc,
        0 AS min_token_age,
        ...
        3 AS rise_filter,
        30 AS rise_filter_time,
        20 AS dump_percent,
        true AS include_pump,
        true AS include_launchlab,
        false AS include_meteora,
...
WHERE 
    r.real_count >= 50
    -- AND r.safe_count >= 3
    AND r.scam * 100 / r.real_count <= 30
    AND r.creator_count * 100 / r.real_count <= 30
    -- AND r.good_count >= 2
    -- AND r.bonding_count > 5
    -- AND r.rise_cond_met >= 50
    -- AND r.hold_rise_3x >= 30
    -- AND r.hold_rise_2x >= 50
    AND r.last_trade >= TIMESTAMP '2025-09-30'
    
    -- AND r.first_trade >= TIMESTAMP '2025-09-20'
ORDER BY r.totalProfit DESC