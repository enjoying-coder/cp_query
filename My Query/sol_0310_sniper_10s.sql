...
constants AS (
    SELECT
        TIMESTAMP '2026-01-02' AS startDate,
        0 AS min_buy_mc,
        40 AS max_buy_mc,
        0 AS min_token_age,
        10 AS max_token_age,
...
creators AS (
    SELECT 
        ...
        block_time,
        dev
    FROM dune.sutomodev_team_9716.result_creators
...
initialTradeTokens_temp AS (
    SELECT
        ...
        date_diff(
            'second',
            COALESCE(c.block_time, TIMESTAMP '2026-02-01'),
            fb.buy_block_time
        ) AS token_age,
    ...
    LEFT JOIN dune.sutomodev_team_9716.result_fake_tokens fake ON ta.token = fake.token
