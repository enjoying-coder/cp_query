...
constants AS (
    SELECT
        (NOW() - INTERVAL '20' day) AS startDate,
        85 AS sol_price
),
sol_price_table AS (
    SELECT 
...
trades AS (
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE mbt.base_amount / COALESCE(spt.price, lsp.latest_price, (SELECT sol_price FROM constants))
        END AS sol_amount,
        mbt.token_amount, 
        CASE 
            WHEN mbt.quote_mint = 'sol' THEN mbt.base_amount * COALESCE(spt.price, lsp.latest_price, (SELECT sol_price FROM constants)) * mbt.supply / mbt.token_amount / 1000
        ...
        mbt.block_slot,
        CASE 
            WHEN mbt.quote_mint = 'sol' THEN mbt.base_amount * COALESCE(spt.price, lsp.latest_price, (SELECT sol_price FROM constants))
