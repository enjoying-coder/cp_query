...
constants AS (
    SELECT
        TIMESTAMP '2025-11-13' AS startDate,
        0 AS min_buy_mc,
        40 AS max_buy_mc,
        0 AS min_token_age,
        600 AS max_token_age,
...
creators AS (
    SELECT 
        ...
        block_time,
        dev
    FROM dune.schoolelite564_team_2039.result_creators
...
initialTradeTokens_temp AS (
    ...
    LEFT JOIN dune.schoolelite564_team_2039.result_fake_tokens fake ON ta.token = fake.token
...
WHERE 
    r.real_count >= 2
    AND r.scam < 1
    AND r.last_trade >= TIMESTAMP '2025-10-05'
